...
def sample(ds, vocab, video_model, n_samples, split='train', verbose=1):
    ...
    for i, (truth_X, truth_Y) in enumerate(zip(truth_Xs, truth_Ys)):
        try:
            ...
            fake_Xs =  video_model.decode_predictions_beam_search(np.asarray(fake_X[-2]), vocab, verbose=0, pad_sequences=True)
            ...
            if shuffled_correctness_list[action]:
                ...
                misses += 1
                if verbose:
                    print "Not correct!. The correct one was:", shuffled_answer_list[shuffled_correctness_list.index(True)]
            guesses += 1
            print ""
            print ""
